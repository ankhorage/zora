...
function readRegistryKeys(): readonly string[] {
  const source = readFileSync(path.join(sourceRoot, 'registry.ts'), 'utf8');
  const registryBody = source.match(
    /const _ZORA_COMPONENT_REGISTRY = \{([\s\S]*?)\n\} as const satisfies ZoraComponentRegistry;/u,
  )?.[1];

  if (!registryBody) {
    throw new Error('Could not locate _ZORA_COMPONENT_REGISTRY in src/registry.ts.');
  }
...
test('prints the canonical ZORA component registry snapshot', async () => {
  ...
  const corePluginSource = readFileSync(path.join(sourceRoot, 'corePlugin.ts'), 'utf8');
  const { ZORA_COMPONENT_META } = await import('../src/metadata');
